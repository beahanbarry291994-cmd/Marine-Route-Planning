...
def fetch_map_with_cache(start_lonlat, goal_lonlat, zoom):
    ...
    print("从 CartoDB 无标签底图下载瓦片（稳定可靠）...")
    ...
    MAP_TOP_LEFT = num2deg(x_min, y_min, zoom)      # (lat, lon)
    MAP_TOP_LEFT = (MAP_TOP_LEFT[1], MAP_TOP_LEFT[0])  # 转为 (lon, lat)
        ...
        for x in range(x_min, x_max + 1):
            ...
            for attempt in range(3):
                try:
                    ...
                    print(f"瓦片 ({x},{y}) 下载失败，重试 {attempt+1}/3...")
                    time.sleep(1.5)
            if tile is None:
...
def process_map_to_grid(start_lonlat, goal_lonlat):
    ...
    # 最终陆地 = 原陆地 或 非最大水域（假湖→陆地）
    land_final = (land_no_bridges == 255) | (real_sea == 0)
    grid_map = land_final.astype(np.uint8)        # 1:陆地, 0:水域
...
# ==========================================
# 6. 主流程与可视化（已修复黑白反转、中文标题）
# ==========================================
def main():
    print("1. 地图获取与预处理（OTSU + 去桥 + 连通域去假湖）...")
    ...
    if ocean_label is None or labels is None:
        print("❌ 初始化失败：无法识别海洋。")
        plt.figure("图1 栅格地图")
        plt.imshow(1 - grid_map, cmap='gray')      # 反转：白水黑陆
        plt.title("全图视为陆地，无水域")
        plt.axis('off')
        plt.show()
        return
    ...
    if not path_px:
        ...
        plt.plot(start_px[0], start_px[1], 'go', markersize=8)
        plt.plot(goal_px[0], goal_px[1], 'ro', markersize=8)
        plt.title("未找到路径")
        plt.axis('off')
        plt.show()
        return
